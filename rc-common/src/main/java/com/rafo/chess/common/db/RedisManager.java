...
public class RedisManager {
    ...
    public void init(Properties props) {
        if (pool == null) {
            try {
                ...
                int db =  Integer.parseInt(props.getProperty("redis.db","0"));
    ...
    public  List<String> hMGetWithException (String key, String ... field) throws PersistException {
        Jedis jedis = null;
        try{
            jedis = pool.getResource();
            return jedis.hmget(key,field);
            ...
            e.printStackTrace();
            throw new PersistException("store key ["+key+"] to redis error");
        }finally {
            if(jedis != null){
                jedis.close();
            }
    ...
    public String hMSetWithException(String key, Map<String, String> map) throws PersistException {
        Jedis jedis = null;
        try{
            jedis = pool.getResource();
            return jedis.hmset(key, map);
